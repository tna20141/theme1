...
if ( ! function_exists( 'theme1_setup' ) ) :
	...
	function theme1_setup() {
		...
		register_nav_menus( array(
			...
			'menu-footer-links' => esc_html__('Footer Links', 'theme1'),
		) );
...
function theme1_scripts() {
	...
	// for production, use the below instead:
	$stylesheet_dir_uri = get_stylesheet_directory_uri();
	$stylesheet_uri = apply_filters('stylesheet_uri', $stylesheet_dir_uri . '/style.min.css', $stylesheet_dir_uri);
	wp_enqueue_style( 'theme1-style', $stylesheet_uri );
...
/**
 * Add a taxonomy for post grouping
 */
function define_topic_taxonomy() {
	register_taxonomy(
		'topic',
		'post',
		array(
			'hierarchical' => false,
			'label'        => __('topic', 'theme1'),
			'query_var'    => true,
			'rewrite'      => true
		)
	);
